...
**Version 14** — Automatic test request arrival: 2-6 new requests/day appear on their own, deterministically distributed across the 4 interactive labs. Requests that sit unscheduled for 4 sim-days expire (shown in red); expired requests auto-archive after a further 7 days, keeping the active queue bounded while preserving full history on the Projects page.
...
```bash
npm test          # Vitest — 93 unit tests on the reducer, selectors, scheduling logic, and result engine
...
Five real bugs have been found and fixed by writing these tests so far, not just by manual clicking:
